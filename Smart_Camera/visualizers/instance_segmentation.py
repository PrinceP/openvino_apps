...
class InstanceSegmentationVisualizer:
    ...
    def __call__(self, image, boxes, classes, scores, masks=None, ids=None, texts=None):
        ...
        if masks is not None:
            result = self.crop_masks(result, masks, ids)
        if self.show_boxes:
    ...
    def crop_masks(self, image, masks, ids=None):
        aggregated_mask = np.zeros(image.shape[:2], dtype=np.uint8)
        aggregated_colored_mask = np.zeros(image.shape, dtype=np.uint8)
        ...
        # Create a new blank image to hold the cropped mask image
        cropped_mask = np.zeros(image.shape, dtype=np.uint8)

        # Use the aggregated mask to crop the mask image from the original image
        x, y, w, h = cv2.boundingRect(aggregated_mask)
        cropped_mask = image[y:y+h, x:x+w].copy()
        cropped_mask[np.logical_not(aggregated_mask[y:y+h, x:x+w])] = 255
